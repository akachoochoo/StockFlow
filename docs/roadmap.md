...
> 마지막 업데이트: 2026-05-17 (Phase 0.11.g 정식 종료 — 라운드 #29, ADR 0014 §1+§2+§3 박제, G1 PASS / G2 FAIL (H1 alpha ≤ 0, H2 monotonic), lifecycle = 5th ring 영구 (permanent-research-only), 회고 `phase-0.11.g.md` + Core-Satellite runner + Asymmetric Grid runner + 16-config sweep CLI, 핵심 발견: 리밸런싱 알파 없음 + 비대칭 grid MDD breach + Phase 1 권고 정적 배분, 28 new tests, 1523 → 1551 tests 회귀 zero 보존)
