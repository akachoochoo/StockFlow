...
> 마지막 업데이트: 2026-05-17 (Phase 0.11.i 정식 종료 — ADR 0016 §1+§3 박제, 게이트 4/4 PRIMARY PASS, lifecycle = permanent (interactive) + archival-with-sunset (static), 회고 `phase-0.11.i.md` + vendored lightweight-charts JS + `_interactive_chart.py` + kakao_dgt_backtest.py + _dgt_renderer.py interactive 전환 + fmt kwarg + 75 new tests, 1575 → 1650 tests 회귀 zero 보존)
...
| **Phase 0.11.d** | 완료 (2026-05-13, 라운드 #26 + sub-step .1~.5 5 commits) — Asset-Specific Strategy Differentiation Diagnosis & Design. **게이트 4/4 PRIMARY PASS** (G1 5 layer 진단 보고서 / G2 D3 4 후보 + D4 3 후보 + D7 16-cell 매트릭스 / G3 ADR + 회고 + supersede template / G4 namespace + mypy + D10 lifecycle). Lifecycle = deferred reference. 본질 = *분석 phase 의 분석 phase* — 코드 변경 zero / 신규 test zero / 박제 산출만 (~1711 docs lines). 핵심 발견: `per_asset_strategy_overrides` (application:187) + `AssetContext` (use_cases:37-57) 기존 구현 발견 ("구조는 있지만 정책 게이트로 차단"). D11 AND-gate 4/4 충족 — 후속 결정 라운드 (가칭 Phase 0.11.f / Phase 1 ADR 0012) 진입 자격 완성. CLAUDE.md §16.1 항목 #2 정정 권고 박제 (옵션 Z — 정정 commit 안 함) | ADR 0010 §1+§2+§3 / `phase-0.11.d.md` + `docs/analysis/phase-0.11.d-{current-structure-diagnosis,coupling-model-candidates}.md` |
| **Phase 0.11.e** | 완료 (2026-05-14, 라운드 #27 + sub-step .1~.5 5 commits + 별도 2 follow-up commits) — Dynamic Adjustment Proposal Engine (L2/L3). **게이트 4/4 PRIMARY PASS** (G1 Proposal 도메인 + L2/L3 trigger evaluator / G2 사람 승인 워크플로우 e2e / G3 ADR + 회고 + Design Contract 박제 / G4 namespace + mypy + 재현성 + Reflexive overfitting 방지). Lifecycle = 5th ring 영구 유지 (D13). 신규 5th ring `src/research/dynamic_adjustment/` 10 파일 (~1403 LOC, _proposal/_proposal_history/_trigger_evaluator/_store/_adr_template/cli + enums + change_spec) + 141 tests (1307→1448, regression zero). §1.6 Design Contract 6 invariant **측정 가능 verification** 완성 (proposal_auto_apply_blocked / no_realtime_data_identifiers / reflexive_data_isolation / adr_enforcement_blocks_apply / trigger_signal_enum_exhaustive / sell_proposal_blocked_without_adr0010). D14 structural constraints (12주 / ±30% / cooldown) + D15 governance "정신 supersede 불가" 박제. **Phase 1 진입 ready 선언** | ADR 0011 §1+§2+§3 / `phase-0.11.e.md` |
| **Phase 0.11.f** | 완료 (2026-05-15, 라운드 #28) — DGT Paper-Faithful + Adaptive Hybrid + Multi-Asset Portfolio. **게이트 4/4 PRIMARY PASS** (G1 75 new tests, 173 total, 0 failures / G2 17 tests pykrx-free / G3 ADR 0013 S1+S2+S3 complete / G4 retrospective + CLAUDE.md + namespace). Lifecycle = 5th ring 영구 (permanent-research-only). 4 runner (dynamic/adaptive/paper/paper_adaptive) + multi-asset portfolio B&H 벤치마크 + CLI 1030 LOC. 핵심 발견: **MDD 14-16% 일관성** (시장 regime/종목 수 무관, B&H MDD 27-34% 약 2배) + Hyb-Daily = 최적 DGT 구성 (ATR×1.0, k=[0.5%,5%], daily, m=n//2) + 상승장 B&H 압도적 우위 (DGT 구조적 한계 — grid level 통과 시 매도). D1~D10 (3 accepts, 3 invalidations, 4 operational). Invalidation catalog: slope/trend (D3), on-breach standalone (D4), narrow grid k<1% (X3). 75 new tests (1448→1523, regression zero). Inner ring 변경 zero | ADR 0013 §1+§2+§3 / `phase-0.11.f.md` |
| **Phase 0.11.g** | 완료 (2026-05-17, 라운드 #29) — DGT Rebalancing Alpha (Core-Satellite + Asymmetric Grid). G1 PASS / **G2 FAIL** (H1 rebalancing alpha ≤ 0, H2 Calmar monotonic, H3 asymmetric MDD 23-25% breach). Lifecycle = 5th ring 영구 (permanent-research-only). Core-Satellite runner (`_HybridCoreRunner` — B&H core + DGT satellite + drift/calendar/static rebalancing) + Asymmetric Grid runner (`_AsymmetricGridRunner` — sell 2-3x spacing + trailing stop) + 16-config sweep CLI. 핵심 발견: **리밸런싱 알파 없음** (bull -6~-12%, bear 근소 음수 — B&H 승자 매도→DGT 투입 = 가치 파괴) + 비대칭 grid 상승장 1.8x 수익 개선이나 하락장 MDD 18% hard stop 초과 + **Phase 1 자본 배분 권고: 정적 배분, DGT = MDD 방어 전용 도구**. Invalidation catalog: rebalancing into DGT (value destruction), asymmetric grid (MDD breach), trailing stop (too late). 28 new tests (1523→1551, regression zero). Inner ring 변경 zero | ADR 0014 §1+§2+§3 / `phase-0.11.g.md` |
| **Phase 0.11.h** | 완료 (2026-05-17) — DGT Backtest Report Chart Upgrade (Candlestick + Volume). **게이트 4/4 PRIMARY PASS** (G1 kakao_dgt_backtest.py 캔들+거래량 12/12 / G2 _dgt_renderer.py 캔들+거래량 22/22 / G3 ADR 0015 + 회고 + figure parade / G4 namespace + regression zero). Lifecycle = permanent. 신규 `src/research/dgt/_candles.py` (공유 helper: `_draw_candles` + `_draw_volume`) + `kakao_dgt_backtest.py` + `_dgt_renderer.py` 업그레이드 (캔들+거래량 2-panel + bar-count-adaptive width `min(40,max(14,n/55))`). ADR 0009 D4 evolution (not supersede) — mplfinance 미사용, manual matplotlib primitives, Option A synthesis. 24 new tests (1551→1575, regression zero). Inner ring 변경 zero | ADR 0015 §1+§3 / `phase-0.11.h.md` |
