...
#   FETCH_MINUTE_SLEEP_SEC    : 종목 간 sleep 초 (기본 = 1.0, 근본 mitigation =
#                               anchor 간 100ms sleep, ADR 0023 R3 4차 실증).
#                               cron wrapper sleep_sec 증가는 무의미했음 —
#                               근본 원인 = anchor burst (KISClient 50ms throttle
#                               이 평균 20 req/s limit 정확히 일치, burst 보호 zero).
#
# 동작:
