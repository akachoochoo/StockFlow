...
#### Phase 0.7.2 (진행 중, 2026-05-04 진입) — 자본 배분 정책 비교
- 종목 유지 (069500 + 214980) — 변수 통제 (배분 정책 1 차원만 변경)
- 배분 정책 후보: 균등 (baseline) / 역변동성 (1/σ 가중) / 정변동성 (σ 가중)
- 게이트 baseline: H1 ≥ 15.6% / H2 ≥ +5.25% / H3 ≥ 0.33 (Phase 0.7.1 측정값, 통과 기준 ≥ 2 참)
- per-asset budget 정적 / 정책 F (drop=5%, target=10%, cooldown=60) 그대로
- 결정: ADR 0003 §16. 회고: `docs/retrospectives/phase-0.7.2.md` (예정).

#### Phase 0.7.3 (예정) — 종목 다양화
- 3 ~ 5 종목 / 정책-자산 부정합 처방 (회고 §6.1 후보 #1)
- 종목별 다른 정책 허용 여부 (ADR §7.3 후속, §14.7 γ 재검토)

#### Phase 0.7 종료 결정 (예정)
- 0.7.3 종료 후 별도 결정 라운드 (ADR §10.2 후속)
- 게이트 통과 / 부분 통과 / 미통과 분기 — §15.5.1 옵션 A 정신상 어느 경우든 Phase 0.8 진입
...
- 보조지표 기반 매수 / 매도 (MA, BB, RSI 등) — Phase 0.8 (가칭, ADR 0003 §15.2)
- 지지선 기반 매수 패러다임 — Phase 0.8 (가칭)
- 개별 주식 검증 (호가 가변, 거래 정지, 액면분할, 거래세) — Phase 0.9 (가칭, ADR 0003 §15.3)
...
*마지막 업데이트: 2026-05-04 (§14 Phase 0.7.1 완료 + Phase 0.7.2 진입 + Phase 0.8/0.9 (가칭) 후보 박제 — ADR 0003 §15.5.1 / §16 후속)*